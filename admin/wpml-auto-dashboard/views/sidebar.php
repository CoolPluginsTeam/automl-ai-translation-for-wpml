...
if ( ! function_exists( 'wpml_auto_format_time_taken' ) ) :
	...
	function wpml_auto_format_time_taken( $time_taken ) {
		if ( 0 === intval( $time_taken ) ) {
			return esc_html__( '0', 'wpml-auto-translate-addon' );
		}
		...
		if ( $time_taken < 60 ) {
			...
			return sprintf( esc_html__( '%d sec', 'wpml-auto-translate-addon' ), $time_taken );
		}
		...
		if ( $time_taken < 3600 ) {
			...
			return sprintf( esc_html__( '%1$d min %2$d sec', 'wpml-auto-translate-addon' ), $min, $sec );
		}

		$hours = floor( $time_taken / 3600 );
		$min   = floor( ( $time_taken % 3600 ) / 60 );
		// translators: 1: hours, 2: minutes.
		return sprintf( esc_html__( '%1$d hours %2$d min', 'wpml-auto-translate-addon' ), $hours, $min );
	}
endif;
	...
	function wpml_auto_get_plugin_display_name( $plugin_slug ) {
		...
		$plugin_paths = array(
			'automatic-translator-addon-for-loco-translate' => array(
				...
				'free_name' => esc_html__( 'LocoAI – Auto Translate For Loco Translate', 'wpml-auto-translate-addon' ),
				'pro_name'  => esc_html__( 'LocoAI – Auto Translate for Loco Translate (Pro)', 'wpml-auto-translate-addon' ),
			),
		);
...
if ( ! function_exists( 'wpml_auto_format_number' ) ) :
	...
	function wpml_auto_format_number( $number ) {
		...
		if ( $number >= 1000000000 ) {
			return round( $number / 1000000000, 1 ) . esc_html__( 'B', 'wpml-auto-translate-addon' );
		} elseif ( $number >= 1000000 ) {
			return round( $number / 1000000, 1 ) . esc_html__( 'M', 'wpml-auto-translate-addon' );
		} elseif ( $number >= 1000 ) {
			return round( $number / 1000, 1 ) . esc_html__( 'K', 'wpml-auto-translate-addon' );
		}
...
<div class="wpml-auto-dashboard-sidebar">
	<div class="wpml-auto-dashboard-status">
		<h3><?php esc_html_e( 'Auto Translation Status', 'wpml-auto-translate-addon' ); ?></h3>
		<div class="wpml-auto-dashboard-sts-top">
			...
			<span><?php esc_html_e( 'Total Characters Translated!', 'wpml-auto-translate-addon' ); ?></span>
		</div>
		<ul class="wpml-auto-dashboard-sts-btm">
			<li>
				<span><?php esc_html_e( 'Total Strings', 'wpml-auto-translate-addon' ); ?></span>
				<span><?php echo esc_html( wpml_auto_format_number( $totals['string_count'] ) ); ?></span>
			</li>
			<li>
				<span><?php esc_html_e( 'Total Translation Jobs', 'wpml-auto-translate-addon' ); ?></span>
				<span><?php echo esc_html( $totals['translation_count'] ); ?></span>
			</li>
			<li>
				<span><?php esc_html_e( 'Time Taken', 'wpml-auto-translate-addon' ); ?></span>
	...
	<div class="wpml-auto-dashboard-translate-full">
		<h3><?php esc_html_e( 'Other Auto Translation Addons', 'wpml-auto-translate-addon' ); ?></h3>
		<div class="wpml-auto-dashboard-addon first">
			<div class="wpml-auto-dashboard-addon-l">
				...
				<span class="addon-desc">
					<?php esc_html_e( 'Loco Translate addon to automatically translate plugins and themes.', 'wpml-auto-translate-addon' ); ?>
				</span>

				<?php if ( wpml_auto_is_plugin_installed( 'automatic-translator-addon-for-loco-translate' ) ) : ?>
					<span class="installed"><?php esc_html_e( 'Installed', 'wpml-auto-translate-addon' ); ?></span>
				<?php else : ?>
					<a href="<?php echo esc_url( admin_url( 'plugin-install.php?s=Automatic+translate+addon+for+loco+translate+by+coolplugins&tab=search&type=term' ) ); ?>"
						...
						target="_blank">
						<?php esc_html_e( 'Install', 'wpml-auto-translate-addon' ); ?>
					</a>
				<?php endif; ?>
			</div>
			<div class="wpml-auto-dashboard-addon-r">
				<img src="<?php echo esc_url( WPML_AT_PLUGIN_URL . 'admin/wpml-auto-dashboard/images/atlt-logo.png' ); ?>" alt="<?php esc_attr_e( 'Loco Auto Translate Addon', 'wpml-auto-translate-addon' ); ?>">
			</div>
		</div>
	</div>

	<div class="wpml-auto-dashboard-rate-us">
		<h3><?php esc_html_e( 'Rate Us ⭐⭐⭐⭐⭐', 'wpml-auto-translate-addon' ); ?></h3>
		<p><?php esc_html_e( "We'd love your feedback! Hope this addon made WPML auto-translations easier for you.", 'wpml-auto-translate-addon' ); ?></p>
		<a href="https://wordpress.org/support/plugin/wpml-auto-translate-addon/reviews/#new-post"
			class="review-link"
			target="_blank">
			<?php esc_html_e( 'Submit a Review →', 'wpml-auto-translate-addon' ); ?>
		</a>
	</div>
</div>