	...
	public function editor_exclude_posts( $exclude_post_ids, $post_ids ) {
		...
		if(is_array($post_ids) && !empty($post_ids)) {
			...
			if(isset($current_post_id)){
				...
				$status = get_post_meta( $post_id, '_automl_translation_editor_native', true );
				if($post_id == $status) {
					$exclude_post_ids[$current_post_id] = __( 'This post was translated using AutoML AI Translation and cannot be translated by WPML. There is no need to open the WPML Advanced Translation Editor.', 'automl-ai-translation-for-wpml' );
				}
			};
		}
