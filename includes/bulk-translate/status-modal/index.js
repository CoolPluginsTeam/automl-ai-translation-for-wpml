...
const StatusModal = ({ postIds, selectedLanguages, prefix, onDestory }) => {
    ...
    const getBulkStatus = () => {
        switch (bulkStatus) {
            case 'running':
                return __('In Progress', 'autopoly-ai-translation-for-polylang-pro');
            case 'pending':
                return __('Pending', 'autopoly-ai-translation-for-polylang-pro');
            case 'completed':
                return __('Completed', 'autopoly-ai-translation-for-polylang-pro');
            default:
                return __('Status', 'autopoly-ai-translation-for-polylang-pro');
        }
    }
    ...
    return (
        errorModal ? <ErrorModalBox message={errorModalData.errorHtml} onClose={closeErrorModal} Title={__('Bulk Translation Error', 'autopoly-ai-translation-for-polylang-pro')} prefix={prefix} >
            {errorModalData.aiError && <div className={`${prefix}-ai-error-buttons`}>
                <button className={`${prefix}-ai-error-button button`} data-status="translateAgain" onClick={AIErrorBtnHandler}>{__('Translate', 'autopoly-ai-translation-for-polylang-pro')}</button>
                <button className={`${prefix}-ai-error-button button`} data-status="continue" onClick={AIErrorBtnHandler}>{__('Continue', 'autopoly-ai-translation-for-polylang-pro')}</button>
            </div>}
        </ErrorModalBox> :
                            ...
                            <>
                                ...
                                {charactersCountVisibility &&
                                    <div className={`${prefix}-translator-strings-count`}>
                                        {__('Wahooo! You have saved your valuable time via auto translating', 'autopoly-ai-translation-for-polylang-pro')}
                                        <strong className="totalChars"> {countInfo.charactersTranslated} </strong>{__('characters using', 'autopoly-ai-translation-for-polylang-pro')}
                        ...
                        <div className={`${prefix}-status-table-container`}>
                            <div>
                                <table className={`${prefix}-status-table`}>
                                    <thead>
                                        <tr>
                                            <th>{__('Language', 'autopoly-ai-translation-for-polylang-pro')}</th>
                                            <th>{__('Status', 'autopoly-ai-translation-for-polylang-pro')}</th>
                                            <th>{__('Title', 'autopoly-ai-translation-for-polylang-pro')}</th>
                                            <th>{__('Actions', 'autopoly-ai-translation-for-polylang-pro')}</th>
                                        </tr>
                                    </thead>
                                                ...
                                                return (
                                                    <React.Fragment key={key}>
                                                        <tr key={`group-title-${key}`} className={`${prefix}-group-title`}>
                                                            <td colSpan="5">
                                                                {errorPostsInfo[key]?.title || __('Untitled', 'autopoly-ai-translation-for-polylang-pro')}
                                                            </td>
                                                        </tr>
                                                        <tr key={key}>
