    ...
    const translatePost = async (index) => {
        ...
        if (postContent) {
            ...
            for (const lang of languages) {
                ...
                storeDispatch(updateTranslatePostInfo({ [postId + '_' + lang]: { status: 'error', messageClass: 'error', errorMessage: __('This post editor type is not supported for translation', 'autopoly-ai-translation-for-polylang-pro') } }));
            }
...
export const updateContent = async ({ source, postId, sourceLang, lang, editorType, createTranslatePostNonce, storeDispatch }) => {
    ...
    await fetch(bulkTranslateRouteUrl + `/${postId}/${endPoint}`, {
        ...
        if (data.success && data.data.post_id) {
            ...
            data.data.post_title = '' === data.data.post_title ? __('N/A', 'autopoly-ai-translation-for-polylang-pro') : data.data.post_title;
...
const bulkTranslateEntries = async ({ ids, langs, storeDispatch }) => {
    ...
    if (!untranslatedPostsData) {
        return { success: false, message: __('No posts to translate data undefined', 'autopoly-ai-translation-for-polylang-pro') };
    }
    ...
    if (!untranslatedPostsData.data) {
        return { success: false, message: __('No posts to translate untranslated data not found', 'autopoly-ai-translation-for-polylang-pro') };
    }

    if (!untranslatedPostsData.data.posts) {
        return { success: false, message: __('No posts to translate untranslated posts data not found', 'autopoly-ai-translation-for-polylang-pro') };
    }

    if (!untranslatedPostsData.data.CreateTranslatePostNonce) {
        return { success: false, message: __('No create translate post nonce', 'autopoly-ai-translation-for-polylang-pro') };
    }
            ...
            try {
                ...
                if (translationResponse?.success && translationResponse.data) {
                    ...
                    batch.forEach((str, index) => {
                        ...
                        batchToSave.push({ field_key: str.field_key, translated: translatedText });
                    ...
                    const totalStrings = Object.values(totalPerLanguage).reduce((a, b) => a + b, 0);
                    storeDispatch(updateProgressStatus((100 * batch.length) / totalStrings));
                    ...
                    storeDispatch(updateTranslatePostInfo({
                        [langKey]: { status: 'error', messageClass: 'error', errorMessage: errorMsg },
                    }));
                    storeDispatch(unsetPendingPost(langKey));
                    storeDispatch(updateCompletedPosts([langKey]));
                    return;
                ...
                const errorMsg = err?.message || __('Translation failed', 'wpml-auto-translate-addon');
                storeDispatch(updateTranslatePostInfo({
                    [langKey]: { status: 'error', messageClass: 'error', errorMessage: errorMsg },
                }));
                storeDispatch(unsetPendingPost(langKey));
