...
class AIService {
    ...
    processChunkBatch = async () => {
        ...
        try {
            ...
            if (error.name === 'AbortError') {
                const errorMessage = this.errorMessage && this.errorMessage.includes('You exceeded your current quota') ? __('You have exceeded you current plan limit. that\'s why the request is aborted.', 'autopoly-ai-translation-for-polylang-pro') : error;
    ...
    pendingRequest = () => {
        ...
        if (limitExceeded) {
            errorMessage = `<p class="${this.prefix}-ai-pending-request-heading">` + __('You’ve exceeded your current plan limit.', 'autopoly-ai-translation-for-polylang-pro') + '</p> ' + __('To continue, please check your plan details and update your API key.', 'autopoly-ai-translation-for-polylang-pro');
            translateBtnMessage = __('Click "Translate" after updating your API key to re-translate the remaining strings.', 'autopoly-ai-translation-for-polylang-pro');
        } else {
            errorMessage = `<p class="${this.prefix}-ai-pending-request-heading">` + __('Oops! Something went wrong during translation', 'autopoly-ai-translation-for-polylang-pro') + '</p>';
            translateBtnMessage = __('Click "Translate" to re-translate the remaining strings.', 'autopoly-ai-translation-for-polylang-pro');
        }

        const message = `<div class="${this.prefix}-ai-pending-request">
                    <div>${errorMessage}</div>
                    <p>${__('To see more details, open your browser’s developer console.', 'autopoly-ai-translation-for-polylang-pro')}</p>

                <p>✅ ${sprintf(__('You’ve translated %s of the strings.', 'autopoly-ai-translation-for-polylang-pro'), completedPercent + '%')}</p>
                <p>❌ ${sprintf(__('%s of the strings are still not translated.', 'autopoly-ai-translation-for-polylang-pro'), notCompletedPercent + '%')}</p>

                <p><strong>${__('Next Steps:', 'autopoly-ai-translation-for-polylang-pro')}</strong></p>

                <p>${translateBtnMessage}</p>
                <p><strong>${__('OR', 'autopoly-ai-translation-for-polylang-pro')}</strong></p>
                <p>${__('Click "Continue" to proceed without translating the rest of the strings.', 'autopoly-ai-translation-for-polylang-pro')}</p>
                </div>`;


        this.storeDispatch(updateTranslatePostInfo({ [this.activePostId + '_' + this.activeTargetLangs]: { status: 'error', messageClass: 'error', errorMessage: __('Translation failed.', 'autopoly-ai-translation-for-polylang-pro'), errorHtml: message, aiError: true, nonce: this.createTranslatePostNonce, completedStrings, totalPosts: this.totalPosts} }));
    }
