...
if(class_exists(Update_Block_Config::class)) return;
...
class Update_Block_Config {
    ...
    {
        $response = wp_remote_get( esc_url_raw( AUTOML_AI_PLUGIN_URL . 'includes/wpml/builder/gutenberg/blocks-config/blocks-config.json' ), array(
            'timeout' => 15,
        ) );
    ...
    {
        $response = wp_remote_get( esc_url_raw( AUTOML_AI_PLUGIN_URL . 'includes/wpml/builder/gutenberg/blocks-config/default-support-child-block.json' ), array(
            'timeout' => 15,
        ) );
    ...
    {
        $response = wp_remote_get( esc_url_raw( AUTOML_AI_PLUGIN_URL . 'includes/wpml/builder/gutenberg/blocks-config/attr-default-value.json' ), array(
            'timeout' => 15,
        ) );
