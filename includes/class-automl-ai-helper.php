...
class WPML_AT_Helper {
	...
	public static function get_post_source_language( $post_id, $post_type ) {
		...
		if ( ! $source_lang ) {
			$source_lang = apply_filters( 'automl_default_language', 'en' );
		}
	...
	public static function get_post_translations( $post_id, $post_type ) {
		$trid = apply_filters( 'automl_element_trid', null, $post_id, 'post_' . $post_type );
		...
		return apply_filters(
			'automl_get_element_translations',
			array(),
			$trid,
			'post_' . $post_type
		);
